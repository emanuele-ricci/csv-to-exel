...
function convert(filtered, file_name){

  return new Promise((resolve,reject)=>{

    fs.createReadStream(filtered)
...
exec("ls", async (error, stdout, stderr) => {
  ...
  exec("mkdir Parsed"),(error, stdout, stderr) => {
    if (error) {
      console.log(`error: ${error.message}`);
      return;
    }
  ...
  for(let d=0; d<filtered.length;d++){
    ...
    await convert(filtered[d], file_name).then(res=>console.log(res)).catch(err=>console.log(err));
